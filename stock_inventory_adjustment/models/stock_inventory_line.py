...
class InventoryLine(models.Model):
    ...
    _order = "product_id, location_id, prod_lot_id"
    ...
    product_qty = fields.Float(
        "Counted Quantity", digits="Product Unit of Measure", default=0
    )
    ...
    def _check_no_duplicate_line(self):
        for record in self:
            domain = [
                ...
                ("location_id", "in", record.location_id.ids),
                "|",
                ("partner_id", "in", record.partner_id.ids),
                ("partner_id", "=", None),
                "|",
                ("package_id", "in", record.package_id.ids),
                ("package_id", "=", None),
                "|",
                ("prod_lot_id", "in", record.prod_lot_id.ids),
                ("prod_lot_id", "=", None),
                "|",
                ("inventory_id", "in", record.inventory_id.ids),
                ("inventory_id", "=", None),
            ]
            groupby_fields = [
                "product_id",
            ...
            for group in self.read_group(
                ...
                key = tuple(
                    [group[field] and group[field][0] for field in groupby_fields]
                )
            ...
            key = (
                ...
                record.prod_lot_id.id,
                record.inventory_id.id,
            )
            if lines_count.get(key, 0) > 1:
                raise UserError(
                    _(
                        "There is already one inventory adjustment line for this product, you should rather modify this one instead of creating a new one."
                    )
                )
